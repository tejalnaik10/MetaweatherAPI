...
public class StepDefinition {
	...
	@When("^the system requests (GET|POST|PUT|DELETE) \"([^\"]*)\"$")
	...
	public void the_response_time_is_less_than_seconds(long time) throws Throwable {
		sa.assertTrue(response.getTime()<time);
		sa.assertAll();
	}
	...
	public void the_response_body_contains_weather_for_days(int days) throws Throwable {
		for (int i = 0; i < days; i++) {
			...
			//response.then().assertThat().body("consolidated_weather.applicable_date[" + i + "]",Is.is(dateFormat.format(cal.getTime())));  // hard assert
	...
	public void the_response_body_contains_field_Value(DataTable dt) throws Throwable {
		...
		for (int i = 0; i < list.size(); i++) {
			/// response.then().assertThat().body(list.get(i).get("field"), Is.is(list.get(i).get("value")));   //hard assert
